...
function stopTimer(startTime, patientID, numOfImgs) {
    const stopTime = process.hrtime.bigint();
    const deltaTime = stopTime - startTime;
    storeResult({ startTime, stopTime, deltaTime, patientID, numOfImgs });
}
...
function storeResult(trialData) {
    const { startTime, stopTime, deltaTime, patientID, numOfImgs } = trialData;
    const fullPath = path.join(__dirname, "measures", `${numOfDocuments}_${selectedDatabase}.csv`);
    const csvRow = `${convertTime(startTime)},${convertTime(stopTime)},${convertTime(deltaTime)},${patientID},${numOfImgs}\n`;
